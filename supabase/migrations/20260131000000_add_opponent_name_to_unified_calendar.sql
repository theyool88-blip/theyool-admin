-- Migration: Add opponent_name to unified_calendar VIEW
-- Date: 2026-01-30
-- Description: Adds opponent_name column to display "의뢰인 v 상대방" format consistently
...
SELECT
  ...
  (ch.scourt_raw_data->>'result')::TEXT AS scourt_result_raw,
  ch.scourt_hearing_hash::TEXT AS scourt_hearing_hash,
...
SELECT
  ...
  NULL::TEXT AS scourt_hearing_hash,
  ...
  (SELECT cp.party_name FROM case_parties cp
   ...
   AND cp.id NOT IN (
     SELECT cc.linked_party_id FROM case_clients cc
     WHERE cc.case_id = cd.case_id AND cc.linked_party_id IS NOT NULL
   )
   ORDER BY cp.party_order LIMIT 1
  )::TEXT AS opponent_name,
  -- 정렬 우선순위
...
COMMENT ON VIEW unified_calendar IS '법원기일, 데드라인, 상담, 일반일정을 통합한 캘린더 뷰 (opponent_name 컬럼 추가)';
