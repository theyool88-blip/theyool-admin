-- ============================================================================
-- Migration: Update unified_calendar view to use case_clients table
-- Date: 2026-01-28
-- Description: Replace deprecated is_our_client column with case_clients table
--              for accurate client party name retrieval
-- ============================================================================

-- Drop existing view (required since we're changing column logic)
DROP VIEW IF EXISTS unified_calendar;

-- Recreate unified_calendar view with case_clients integration
...
SELECT
  ...
  -- 당사자 정보 (의뢰인) - Updated to use case_clients table
  COALESCE(
...
SELECT
  ...
  -- 당사자 정보 (데드라인에 연결된 당사자 또는 의뢰인) - Updated to use case_clients table
  COALESCE(
...
COMMENT ON VIEW unified_calendar IS '법원기일, 데드라인, 상담, 일반일정을 통합한 캘린더 뷰 (case_clients 테이블 사용)';

-- ============================================================================
-- Change Summary:
-- 1. Court hearings section: Updated our_client_name to use case_clients + linked_party_id
-- 2. Deadlines section: Updated our_client_name to use case_clients + linked_party_id
-- 3. Added fallback to clients.name when linked_party_id is NULL
-- 4. Prioritizes is_primary_client for consistent client selection
-- ============================================================================
