-- ============================================================================
-- Migration: Add result column to unified_calendar view
-- Date: 2026-01-28
-- Description: Add hearing result (continued, settled, judgment, adjourned, etc.)
--              to enable calendar display of postponed/completed hearings
-- ============================================================================

-- Drop existing view (required since we're changing columns)
DROP VIEW IF EXISTS unified_calendar;

-- Recreate unified_calendar view with result column
...
SELECT
  ...
  -- 당사자 정보 (의뢰인) - case_clients 테이블 사용
  COALESCE(
...
SELECT
  ...
  -- 당사자 정보 (데드라인에 연결된 당사자 또는 의뢰인) - case_clients 테이블 사용
  COALESCE(
...
COMMENT ON VIEW unified_calendar IS '법원기일, 데드라인, 상담, 일반일정을 통합한 캘린더 뷰 (result 컬럼 포함)';

-- ============================================================================
-- Change Summary:
-- 1. Added ch.result::TEXT AS result to COURT_HEARING section
-- 2. Added NULL::TEXT AS result to DEADLINE, CONSULTATION, GENERAL_SCHEDULE sections
-- 3. This enables calendar UI to show postponed/completed hearing results
-- ============================================================================
