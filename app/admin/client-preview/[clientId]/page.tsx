...
export default function ClientPortalPreview() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[var(--bg-secondary)]">
        <div className="text-center">
          <div className="w-10 h-10 border-2 border-[var(--sage-muted)] border-t-[var(--sage-primary)] rounded-full animate-spin mx-auto mb-3"></div>
          <p className="text-[var(--sage-primary)] text-sm">불러오는 중...</p>
        </div>
      </div>
    );
  }
  ...
  if (!client) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[var(--bg-secondary)]">
        <p className="text-[var(--text-tertiary)]">의뢰인 정보를 찾을 수 없습니다.</p>
      </div>
    );
  }
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-[var(--bg-primary)] flex flex-col">
        {/* 헤더 */}
        <header className="bg-[var(--bg-secondary)] border-b border-[var(--border-subtle)] px-5 py-6">
        ...
        <div className="flex-1 flex items-center justify-center px-5">
          <div className="w-full max-w-sm">
            <div className="card rounded-2xl p-8">
              {/* 자물쇠 아이콘 */}
              <div className="w-16 h-16 bg-[var(--sage-muted)] rounded-full flex items-center justify-center mx-auto mb-6">
                <svg className="w-8 h-8 text-[var(--sage-primary)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div className="flex justify-center gap-3 mb-4">
                {[0, 1, 2, 3].map((index) => (
                  <input
                    ...
                    className="form-input w-14 h-14 text-center text-2xl font-bold rounded-xl"
                    autoFocus={index === 0}
                  />
                ))}
              </div>
              ...
              {/* 에러 메시지 */}
              {authError && (
                <p className="text-[var(--color-danger)] text-sm text-center mb-4">{authError}</p>
              )}
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)]">
      ...
      {isAdminPreview && (
        <div className="bg-[var(--sage-primary)] text-white px-4 py-2 text-center text-xs">
        ...
        <div className="max-w-lg mx-auto px-5 py-6">
          ...
          <div>
            <h1 className="text-2xl font-bold text-[var(--text-primary)]">
              {client.name}<span className="font-normal text-[var(--text-muted)]">님,</span>
            </h1>
            <p className="text-[var(--sage-primary)] mt-1 text-sm">{getGreeting()}</p>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-lg mx-auto px-5 py-6 space-y-5">
        ...
        {primaryCase && (
          <>
            {/* 사건 카드 */}
            <section className="bg-[var(--sage-primary)] rounded-2xl p-5 text-white">
            ...
            <section className="card rounded-xl p-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-[var(--bg-primary)] rounded-lg p-3">
                  <p className="text-xs text-[var(--text-muted)] mb-1">사건번호</p>
                  <p className="text-sm font-medium text-[var(--text-primary)]">
                    {primaryCaseHearings[0]?.case_number || '-'}
                  </p>
                </div>
                <div className="bg-[var(--bg-primary)] rounded-lg p-3">
                  <p className="text-xs text-[var(--text-muted)] mb-1">담당판사</p>
                  <p className="text-sm font-medium text-[var(--text-primary)]">
                    {primaryCaseHearings[0]?.judge_name || '-'}
                  </p>
                </div>
                <div className="bg-[var(--bg-primary)] rounded-lg p-3">
                  <p className="text-xs text-[var(--text-muted)] mb-1">사건유형</p>
                  <p className="text-sm font-medium text-[var(--text-primary)]">
                    {primaryCase.case_type || '-'}
                  </p>
                </div>
                    ...
                    return (
                      <div
                        ...
                        className={`card rounded-xl p-4 ${
                          isUrgent ? 'border-[var(--color-danger-muted)]' : ''
                        }`}
                      >
                        <div className="flex items-center gap-3">
                          <div className={`w-12 h-12 rounded-xl flex flex-col items-center justify-center ${
                            isUrgent ? 'bg-[var(--color-danger)]' : 'bg-amber-500'
                          } text-white`}>
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="font-semibold text-[var(--text-primary)] text-sm">{deadline.deadline_type || '기한'}</p>
                            {deadline.description && (
                              <p className="text-xs text-[var(--text-tertiary)] truncate mt-0.5">{deadline.description}</p>
                            )}
                          </div>
            ...
            {primaryCaseHearings.length > 0 && (
              <section>
                <h3 className="text-sm font-bold text-[var(--text-primary)] mb-3 flex items-center gap-2">
                  <span className="w-1 h-4 bg-[var(--sage-primary)] rounded-full"></span>
                  재판기일
                </h3>
                <div className="space-y-2">
                  {/* 예정된 재판 */}
                  {upcomingHearings.map((hearing) => (
                    <div
                      key={hearing.id}
                      className="card rounded-xl p-4 cursor-pointer hover:border-[var(--sage-primary)] transition-colors"
                      onClick={() => setSelectedHearing(hearing)}
                    >
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 bg-[var(--sage-primary)] rounded-xl flex flex-col items-center justify-center text-white">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-0.5">
                            ...
                            <span className="px-2 py-0.5 bg-[var(--sage-muted)] text-[var(--sage-primary)] text-xs font-bold rounded-full">
                  ...
                  {pastHearings.length > 0 && (
                    <div className="pt-2">
                      <p className="text-xs text-[var(--text-muted)] mb-2">지난 재판</p>
                      {pastHearings.map((hearing) => (
                        <div
                          key={hearing.id}
                          className="bg-[var(--bg-tertiary)] rounded-xl p-4 cursor-pointer hover:bg-[var(--bg-hover)] transition-colors mb-2"
                          onClick={() => setSelectedHearing(hearing)}
                        >
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 bg-[var(--border-default)] rounded-lg flex flex-col items-center justify-center">
                              <span className="text-[9px] text-[var(--text-secondary)]">{formatDate(hearing.hearing_date).month}월</span>
                              <span className="text-sm font-bold text-[var(--text-secondary)]">{formatDate(hearing.hearing_date).day}</span>
                            </div>
                            <div className="flex-1 min-w-0">
                              <p className="text-sm text-[var(--text-secondary)]">{formatDate(hearing.hearing_date).full}</p>
                              <div className="flex items-center gap-2 mt-1">
                                ...
                                {hearing.hearing_report && (
                                  <span className="text-xs text-[var(--sage-primary)] flex items-center gap-1">
            ...
            {completedDeadlines.length > 0 && (
              <section>
                <h3 className="text-sm font-medium text-[var(--text-muted)] mb-2 flex items-center gap-2">
                  <span className="w-1 h-4 bg-[var(--border-default)] rounded-full"></span>
                  완료된 기한
                </h3>
            ...
            {primaryCaseHearings.length === 0 && primaryCaseDeadlines.length === 0 && (
              <section className="card rounded-xl p-8 text-center">
                <div className="w-14 h-14 bg-[var(--bg-tertiary)] rounded-full flex items-center justify-center mx-auto mb-3">
                  <svg className="w-7 h-7 text-[var(--text-muted)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                  </svg>
                </div>
                <p className="text-[var(--text-tertiary)] text-sm">현재 예정된 재판이나 기한이 없습니다.</p>
              </section>
            )}
          </>
        )}
          ...
          <section>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-bold text-[var(--text-primary)] flex items-center gap-2">
                <span className="w-1 h-4 bg-[var(--text-muted)] rounded-full"></span>
                다른 사건
              </h3>
              <span className="text-xs text-[var(--text-muted)] bg-[var(--bg-tertiary)] px-2 py-0.5 rounded-full">{otherCases.length}건</span>
            </div>
            <div className="space-y-2">
              {otherCases.map((caseItem) => (
                <Link
                  ...
                  className="block card rounded-xl p-4 hover:border-[var(--sage-primary)] transition-colors"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-[var(--text-primary)] text-sm truncate">{caseItem.case_name}</p>
                      <p className="text-xs text-[var(--text-muted)] mt-0.5">{caseItem.case_type || '-'}</p>
                    </div>
                    <svg className="w-4 h-4 text-[var(--text-muted)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                    </svg>
        ...
        {cases.length === 0 && (
          <section className="card rounded-xl p-8 text-center">
            ...
            <p className="text-[var(--text-tertiary)] text-sm">등록된 사건이 없습니다.</p>
          </section>
        )}
        ...
        <section className="pt-2">
          <div className="card rounded-xl overflow-hidden">
            <div className="p-4 border-b border-[var(--border-subtle)]">
              <p className="text-xs text-[var(--text-muted)] mb-1">문의하기</p>
              <p className="text-sm text-[var(--text-secondary)]">궁금한 점이 있으시면 연락주세요</p>
            </div>
            <div className="grid grid-cols-2 divide-x divide-[var(--border-subtle)]">
              <a
                href={`tel:${officePhone.number.replace(/-/g, '')}`}
                className="flex flex-col items-center gap-2 p-4 hover:bg-[var(--bg-hover)] transition-colors"
              >
                <div className="w-10 h-10 bg-[var(--sage-muted)] rounded-xl flex items-center justify-center">
                  <svg className="w-5 h-5 text-[var(--sage-primary)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                  </svg>
                </div>
                <span className="text-sm font-medium text-[var(--text-primary)]">전화</span>
                <span className="text-xs text-[var(--text-muted)]">{officePhone.display}</span>
              </a>
              ...
              >
                ...
                <span className="text-sm font-medium text-[var(--text-primary)]">카카오톡</span>
                <span className="text-xs text-[var(--text-muted)]">바로 연결</span>
              </a>
            </div>
          </div>
        </section>
      ...
      {selectedHearing && (
        ...
        >
          <div
            className="bg-[var(--bg-secondary)] rounded-t-2xl sm:rounded-2xl w-full max-w-lg max-h-[90vh] overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            {/* 모달 헤더 */}
            <div className="bg-[var(--bg-secondary)] border-b border-[var(--border-subtle)] px-5 py-4">
            ...
            <div className="p-5 overflow-y-auto max-h-[calc(90vh-140px)] space-y-4">
              ...
              <div className="flex items-center gap-4 p-4 bg-[var(--bg-primary)] rounded-xl">
                <div className="w-14 h-14 bg-[var(--bg-secondary)] rounded-xl flex flex-col items-center justify-center shadow-sm">
                  <span className="text-xs text-[var(--text-muted)]">{formatDate(selectedHearing.hearing_date).month}월</span>
                  <span className="text-2xl font-bold text-[var(--text-primary)]">{formatDate(selectedHearing.hearing_date).day}</span>
                </div>
                <div>
              ...
              {getDDay(selectedHearing.hearing_date) >= 0 ? (
                <div className="p-4 bg-[var(--sage-muted)] rounded-xl text-center">
                  <p className="text-sm text-[var(--sage-primary)]">
                    재판까지 <span className="font-bold">D-{getDDay(selectedHearing.hearing_date)}</span> 남았습니다
                  </p>
                </div>
              ) : (
