...
| 조건 | 메시지 |
|-----|--------|
| 재판 2주전 + 제3자 회신 미수령 | "증거회신 미수령: {증거명}" |

**데이터 소스**: SCOURT progress (송달내역)

**증거신청 프로세스**:
1. 당사자가 법원에 증거신청 (사실조회, 문서제출명령 등)
2. 법원이 신청 수용 시 제3자에게 송달 (송달내역에 기록)
3. 제3자가 법원에 회신 제출
...
if (다음기일 - today <= 14일) {
  ...
  송달목록 = progress where (progCttDvs === '4' || 송달 포함)
                        AND (사실조회/제출명령/문서송부촉탁 포함)
                        AND NOT (도착통지/원고/피고 등 당사자 관련)

  for each 송달 {
    if (이후 해당 증거에 대한 제출/회신/회보 없음) {
      → "증거회신 미수령: {증거명}"
    }
  }
}
...
**키워드**:
- 증거조사 송달: `사실조회`, `제출명령`, `문서송부촉탁`, `송부촉탁`
- 제외 (당사자 통지): `도착통지`, `통지서`, `원고`, `피고`, `신청인`, `피신청인`, `대리인`, `변호사`
- 회신 확인: `제출`, `회신`, `회보` (또는 progCttDvs === '3')
