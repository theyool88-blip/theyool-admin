...
export const POST = withTenant(async (request, { tenant }) => {
  try {
    ...
    // client_role_status 결정 로직:
    // 1. 명시적으로 client_role을 지정한 경우: 'confirmed'
    // 2. 원본 사건에서 client_role이 복사된 경우: 'confirmed'
    // 3. 의뢰인과 상대방 성씨가 같은 경우: 'provisional' (역할 확인 필요)
    // 4. 의뢰인과 상대방 성씨가 다른 경우: 'confirmed' (자동 확정)
    let resolvedClientRoleStatus: 'provisional' | 'confirmed' = 'confirmed'
    if (!body.client_role && !sourceCase?.client_role) {
      // 의뢰인 이름 확인
      const clientName = body.new_client?.name
      ...
      // 성씨 비교: 첫 글자가 같으면 provisional
      if (clientName && resolvedOpponentName) {
        const clientSurname = clientName.charAt(0)
        const opponentSurname = resolvedOpponentName.charAt(0)
        if (clientSurname === opponentSurname) {
          resolvedClientRoleStatus = 'provisional'
        }
      }
    }
