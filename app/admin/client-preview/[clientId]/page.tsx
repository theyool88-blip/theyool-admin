  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-amber-500 text-white px-4 py-2 text-center text-sm font-medium">
        관리자 미리보기 모드 - {client.name}님의 의뢰인 포털 화면
        <Link href="/admin/clients" className="ml-4 underline hover:no-underline">
          돌아가기
        </Link>
      </div>
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-3xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-xl font-bold text-gray-900">안녕하세요, {client.name}님</h1>
              <p className="text-sm text-gray-500 mt-1">법무법인 더율</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-4 py-6 space-y-6">
              ...
              {upcomingHearings.map((hearing) => (
                <div key={hearing.id} className="bg-white rounded-lg border border-gray-200 p-4">
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="text-sm font-medium text-gray-900">{hearing.case_name}</p>
                      <p className="text-xs text-gray-500 mt-1">
                        {hearing.hearing_date} {hearing.hearing_time} | {hearing.court_name}
                      </p>
                    </div>
                    <span className="px-2 py-0.5 text-xs rounded-full bg-blue-100 text-blue-700">
                      D-{Math.ceil((new Date(hearing.hearing_date).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24))}
                    </span>
                  </div>
                </div>
              ))}
        ...
        {/* 내 사건 목록 */}
        <section>
          <h2 className="text-sm font-semibold text-gray-900 mb-3">내 사건 ({cases.length})</h2>
          {cases.length === 0 ? (
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-medium text-gray-900 truncate">{caseItem.case_name}</p>
                      ...
                      <div className="flex items-center gap-3 mt-2 text-xs text-gray-400">
                        ...
                        {caseItem.opponent_name && <span>vs {caseItem.opponent_name}</span>}
        ...
        {/* 문의 안내 */}
        <section className="p-4 bg-sage-50 rounded-lg">
